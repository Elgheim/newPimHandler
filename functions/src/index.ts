...
exports.syncVariantGallery = functions.https.onRequest(async (req, res):
Promise<any> => {
  const {method} = req;
...
async function handleImageSync(
    imageIds: ImageIds[], token: token): Promise<imageResponse> {
  ...
  for (let i = 0; i < formattedIds.length; i += 5) {
    await Promise.all([
      ...formattedIds.slice(i, i + 5).map(async (id) => {
        ...
        await data.json() as fetchmediaidbyurlResponse )
            .catch((error: unknown) =>
        ...
        try {
          ...
          await Promise.all(
            ...
            const exsistingArray = variantWp?.meta_data.filter(
                (item: wooMeta) =>
                  item?.key === "_wc_additional_variation_images"
            );
...
async function handleFetchVariantBySku(sku: string) {
  const products =
  await fetch(`${wpUrl}/wc/v3/products?sku=${sku}`, {
    method: "GET",
    headers: {
      Authorization:
        "Basic " +
...
async function updateWCVariantImage(
    parentId: string, variantId: string, wpVariantArray: string) {
  const data = {
    meta_data: [
      {
